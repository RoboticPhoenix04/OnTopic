...
if (isset($_POST["make"])) {
    if (!empty($_POST["title"]) && !empty($_POST["content"])) {
        ...
        $sql = "INSERT INTO blogs (nameuser, iduser, title, content, datecreation) VALUES (?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$name, $_SESSION["loggedInUser"], $_POST["title"], $_POST["content"], $date]);
        header("Location: blogs.php");
    }
}

$query = $pdo->query("SELECT title, content FROM blogs WHERE iduser= {$_SESSION["loggedInUser"]}");
            ...
            <div class="col-6 my-3">
                <form method="post" id="blogform">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="titleinput" name="title" placeholder="Create a catchy title">
                    </div>
                    <div class="form-group">
                        <label for="contentinput">Content</label>
                        <textarea class="form-control" id="contentinput" name="content" rows="20" placeholder="Write something interesting..."></textarea>
                    </div>
                    <button type="submit" class="btn btn-primary my-3" form="blogform" name="make">Create</button>
                </form>
            </div>
        </div>
    </div>
