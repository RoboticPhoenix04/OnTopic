...
if (isset($_POST["delete"])) {
    $query = $pdo->query("DELETE FROM blogs WHERE id= '{$_GET["id"]}'");
    header("Location: blogs.php");
}

$query = $pdo->query("SELECT title, content, nameuser, datecreation FROM blogs WHERE id='{$_GET["id"]}'");
...
<body>
    ...
    <div class="container-fluid mt-1" style="min-height: 100vh;">
        ...
        <div class="row justify-content-center">
            ...
            <div class="col-3">
                <p class="text-end"><?= $result['datecreation'] ?></p>
            </div>
        </div>
        <div class="row justify-content-center">
            <div class="col-6 border-top border-bottom mb-2">
                <p><?= $result['content'] ?></p>
            </div>
        </div>
        <?php if (isset($_SESSION['loggedInUser'])): ?>
            <?php if ($name == $result['nameuser']): ?>
                <div class="row justify-content-center">
                    <div class="col-4 d-flex align-items-center">
                        <span><a href="blogedit.php?id=<?= $_GET['id'] ?>">Edit</a></span>
                    </div>
